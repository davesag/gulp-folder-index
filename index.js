...
module.exports = function(config) {
  ...
  let folderIndex = function(file, enc, callback) {
    ...
    if (file.isNull()) {
      this.emit(EVENTS.error, new PluginError(PLUGIN_NAME, ERRORS.nullFile))
      this.emit(EVENTS.end)
      return callback()
    }

    if (file.isStream()) {
      this.emit(EVENTS.error, new PluginError(PLUGIN_NAME, ERRORS.noStreams))
      this.emit(EVENTS.end)
      return callback()
    }
  ...
  return through.obj(folderIndex, function(cb) {
    if (isEmpty(_index)) {
      this.emit(EVENTS.error, new PluginError(PLUGIN_NAME, ERRORS.emptyFolder))
      this.emit(EVENTS.end)
      return cb()
    }
    ...
    this.push(
      new Vinyl({
        ...
        contents: Buffer.from(JSON.stringify(_index))
      })
    )
